...
def build_alert(entries: list) -> tuple[str, str]:
    ...
    cards = ""
    text = [f"NEW CRYPTO ENTRY SIGNAL(S) — {now}", ""]
    fact_css = "padding:5px 0;font-size:13px;"
    for pair, name, s in entries:
        ...
        rows = "".join(
            ...
            for k, v in [
                ("Price", f"${s['price']:,.0f}  ({s['timeframe']})"),
        ...
        cards += (
            ...
            f'<div style="font-size:16px;font-weight:700;color:#0f172a;">🟢 {coin} — {name.replace("Strategy", "")}'
        ...
        text += [f"{coin} — {name}: ENTRY", f"  price ${s['price']:,.0f}",
...
def main() -> int:
    ...
    for pair, strat_sigs in sigs.items():
        for name, s in strat_sigs.items():
            ...
            state[key] = st
            if st == "ENTRY" and prev.get(key) != "ENTRY":
                new_entries.append((pair, name, s))

    STATE_FILE.write_text(json.dumps(state))

    if not new_entries:
        active = [k for k, v in state.items() if v == "ENTRY"]
        print(f"[alerts] no NEW entry signals (currently active: {active or 'none'}); no email sent")
        return 0

    html, text = build_alert(new_entries)
    subject = f"🟢 Crypto Entry Alert — {len(new_entries)} new signal(s): " + ", ".join(
        f"{_PAIR_META.get(p, {}).get('name', p)}/{n.replace('Strategy', '')}"
        for p, n, _ in new_entries
    )
